...
void sendAMessage(string Username){
    ...
    m.Salt = newSalt();
    while (!flag){
        pw = getpass("Please input your Password: ",true);
        if (pw == getpass("Please re-enter Password: ",true)){
...
int main(int argc, char **argv){
    if (argc == 1){
        try{
            ...
            while(!loggedin){
                cout << "Username: "<< endl;
                cout << "You may be required to sign up if you don't have an account" << endl;
                cin >> Username;
                cin.get();
                //Username = "hi";
